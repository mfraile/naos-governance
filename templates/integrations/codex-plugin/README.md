# NAOS Optional Codex Plugin Adapter

This directory contains adopter-local guidance for teams that want to use the
repo-versioned NAOS Governance Codex plugin with this project.
...
project repo
  ...
  -> Codex plugin installed in Codex
  -> plugin operates the project-local NAOS kit
```

The plugin should be installed or refreshed through Codex's plugin mechanism.
This setup-module copy does not mutate Codex plugin caches, personal marketplace
files, or active IDE settings.
...
If Codex discovers a lesson that should change future behavior, record it as
candidate learning or a review finding first. Promote it through NAOS governed
learning lifecycle before changing plugin skills, Claude Code hooks, Cursor
rules, VS Code/Copilot instructions, Gemini guidance, or MCP declarations.
